    ...
    for ch in xml.chars() {
        match state {
            ...
            ParseState::CloseTag if ch == '>' => {
                if open_tag == key {
                    if open_tag == "item" {
                        let value = content.parse::<f64>().expect(&format!(
...
fn as_angular_velocity(name: &str, value: &str) -> Result<AngularVelocity, Box<dyn Error>> {
    ...
    match result {
        ...
        Err(e) => {
            return Err(
                format!("Failed to parse AngularVelocity {}: {}. {}", name, value, e).into(),
            );
        }
    }
}
...
fn as_electrical_current(name: &str, value: &str) -> Result<ElectricCurrent, Box<dyn Error>> {
    ...
    match result {
        ...
        Err(e) => {
            return Err(
                format!("Failed to parse ElectricCurrent {}: {}. {}", name, value, e).into(),
            );
        }
    }
}
...
fn as_electrical_charge(name: &str, value: &str) -> Result<ElectricCharge, Box<dyn Error>> {
    ...
    match result {
        ...
        Err(e) => {
            return Err(
                format!("Failed to parse ElectricCharge {}: {}. {}", name, value, e).into(),
            );
        }
    }
}
