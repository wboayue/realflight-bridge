...
mod tests {
    ...
    impl AsyncBridge for StubBridge {
        async fn exchange_data(&self, _control: &ControlInputs) -> Result<crate::SimulatorState, BridgeError> {
    ...
    async fn shutdown_via_cancellation_token() {
        ...
        let handle = tokio::spawn(async move {
            server.run_with_bridge(&bridge, server_cancel).await
        });
    ...
    async fn routes_enable_rc_to_bridge() {
        ...
        assert!(matches!(response.status, super::super::super::remote::ResponseStatus::Success));
    ...
    async fn exchange_data_without_payload_returns_error() {
        ...
        assert!(matches!(response.status, super::super::super::remote::ResponseStatus::Error));
