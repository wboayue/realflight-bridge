...
fn bench_extract_element(c: &mut Criterion) {
    c.bench_function("bench_extract_element", |b| {
        b.iter(|| {
            let extracted = extract_element(black_box("m-resetButtonHasBeenPressed"), black_box(SIM_STATE_RESPONSE));
            black_box(extracted)
        })
    });
}
...
criterion_group!(benches, bench_decode_state, bench_extract_element, bench_extract_elements);
